...
#
# dbschema v1.0.6
# modified by Ligon Liu(github.com/ligonliu) from dbschema v1.
...
lowercase = "abcdefghijklmnopqrstuvwxyz" + "ßäöüçáéíóúàèìòùãẽĩõũâêîôûëï"
uppercase = "ABCDEFGHIJKLMNOPQRSTUVWXYZ" + "ÄÖÜÇÁÉÍÓÚÀÈÌÒÙÃẼĨÕŨÂÊÎÔÛËÏ"
...
def upcSplit(s):
    ...
    for c in s:
        if c in uppercase:
            ...
            if attr is None:
                attr = ""  # attribute starting with lower case
            attr += c
    if attr is not None: attrs.add(attr)
    return attrs
...
def mincoverage(abh, scramble=0, hints=None):
    ...
    while len(traverse) > 0:
        ...
        if len(newre) == 0:
            ...
            if len(li) > 1:
                ...
                if lired != li:
                    abh = redabh
                    lired = frozenset(lired)  # left sides need to be unchangeable frozensets in order to serve as dictionary keys
                    if lired in abh:
...
def keysTreeAlg(attr, abh, verbty=None):
    global verbosity
    verbty = verbosity if verbty is None else verbty
